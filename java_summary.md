...
## 内部类以及静态内部类
1. 内部类(嵌套类)在java1.0中没有这个概念，最早是在java1.1提出的，有静态内部类和非静态内部类两种。
2. 匿名内部类是非静态内部类的一种。
3. 内部类定义在外部类中，甚至可以是代码块中(比如某一个方法内部)
...
8. 所有规律或者行为都起源于jsl，java语言规范对所有行为进行了说明，语言设计也遵循这一规范。

## final关键字
1. final类，不能被继承，不会有子类。 //防继承
2. final方法，类中的final方法，表示该方法不能被重写。//防重写
3. final变量，表示该变量生命周期内不可变，只能赋值一次。//防修改
4. final修饰形参，方法内不能修改这个传参拷贝变量，这里值得是这个引用值本身，对应引用对象的成员是可能进行改变的。
...
## 包与成员可见性问题
1. 对于类来说有public和默认两种，公有类：对所有代码都可访问；默认类：同一个包中的代码才可以访问
2. 对于类成员有public、protected、默认成员、以及private私有成员。其中public总是可见的，private只在类内部可见，默认成员只在包内可见，对于包外的类不可见，protected是包内可见、跨包的子类也可见。
...
1. interface可以定义为public或者默认的，隐含是abstrace的，程序中不应在显示地使用abstract修饰interface。
2. 接口中的变量隐式地定义为public static final
3. 接口中的方法被隐式声明为public abstrac
4. java8特性，接口提供默认方法，作为接口方法的默认实现；接口中加入static方法(java8之前不允许这样)，但注意抽象方法与static是不兼容的，因为抽象是重写或者实现后呈现出多态，而静态则是类绑定，不具有动态的效果，只可能发生屏蔽的情况。
5. 默认方法又名扩展方法，java8提出，好处是在不破坏现有代码的情况下对接口扩展，比如java8推出了流式操作，为集合类进行了完美的结合。
